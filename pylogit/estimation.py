...
def calc_and_store_post_estimation_results(results_dict,
    ...
    results_dict["final_hessian"] = np.eye(len(final_params)) #\
        #estimator.convenience_calc_hessian(final_params)
    results_dict["fisher_info"] = np.ones((len(final_params),len(final_params))) # \
...
def estimate(init_values,
    ...
    """
    if not just_point:
        # Perform preliminary calculations    #l676 estimator.zero_vector
    ...
    if ((not estimator.reduced) and (estimator.boxcox)): 
        ...
        for i in range(len(init_values)):
            if (i<3+int((len(init_values)-3)/2)):
                boundaries.append((-np.inf,np.inf))
            else:
                boundaries.append((0,np.inf)) #for > 0
    elif(estimator.reduced):
        for i in range(len(init_values)):
            if(i in estimator.indd):
                if (i<3+int((len(init_values)-3)/2)):
                    boundaries.append((-np.inf,np.inf))
                else:
                    boundaries.append((0,np.inf)) #for > 0
