...
from scipy.special import boxcox as bx
import numpy as np
...
def calc_probabilities(beta,
 ...
 Perform the Box-Cox transformation over the dataset
 Explainations about variables name and computation :
     - nASC : Number of 'Alternative Specific Constant' represents 
     - nlambda : number of lambda paramter. By definition equals to the number
                 of beta parameter.
    n = total length of the 'beta' vector, therefore it is the total number of 
        variables.
    lambdas : From the 'beta' input vector, takes the part which corresponds 
             to lambdas parameter.
    betas : From the 'beta' input vector, takes the part which corresponds 
            to betas parameter.
    X : 2d numpy array, represents the part of the 'design' matrix in input 
        where the Box-Cox transformation must be realised
    X0 : Generate to avoid '0 issues' caused by 0 value in X for lambda=0.
         In this case 'boxcox' method of Scipy returns '-inf'. Thus 
         X0 is defined to remember where there is '0' in X, then X is modified
         to have '1' in place of 0, 'boxcox' of Scipy is called and this 
         result is finally multiply by X0 to re-obtain '0' where it must be. 
         Thanks to this the Xcox matrix is obtained.
    Xcox2 : Concatenation in order to have a matrix with the good number of 
            columns to mutilpily the 'betas' vector. The three columns of 
            'ASC' variables are added to Xcox at the begining of the matrix 
            as no boxcox must be perfomed on. Finally Xcox2 is the Box-cox 
            transformation of 'design' matrix
            
    """
    if(boxcox):
        nASC=np.max(alt_IDs)-1
        nlambda=int((len(beta)-nASC)/2)  
        n=len(beta)
        lambdas=beta[nASC+nlambda:]
        betas=beta[0:nASC+nlambda]
        X=np.copy(design[:,nASC:nASC+nlambda])
        X0=1*(X!=0)
        X[np.where(X==0)] = 1
        Xcox=X0*bx(X,lambdas)    
        Xcox2=np.concatenate((design[:,0:nASC],Xcox),axis=1)
        

        sys_utilities =Xcox2.dot(betas)
    else :
...
def calc_log_likelihood(beta,
    ...
    if (lasso is None) and (ridge is None):                                                #####################################
        ...
        """
 ######################## MODIFIED LINES #####################################
 Performs regularisation over the log-likelihood. Depending on if 'lasso' or 
 'ridge' input parameter are defined, realised a L1, L2 or the two 
 regularisations.

        """
        if (lasso is not None) and ((ridge is not None)):                              #%%  ####################################                       
            
            return log_likelihood - lasso*reg.L1(params,boxcox)-ridge*reg.L2(params)            
        elif(lasso is not None):
            return log_likelihood - lasso*reg.L1(params,boxcox)
        elif(ridge is not None):
            return log_likelihood - ridge*reg.L2(params,boxcox)                              #%%  #####################################
        """
 ######################## END MODIFIED LINES #################################
...
def calc_gradient(beta,
 ...
 Computes the gradient of the log-likelihood with a Box-Cox transformation.
 Explainations about variables name and computation :
     - Line 480-491 : As before, defintion of the Box-Cox transformation
     - L496-504 : Definition of the gradient.
                 For lambdas not zero, the mathematical expression of the 
                 derivative with respect to each lambda is computed. 
                 If lambda=0 as the derivative is ambigous (not continuous)
                 in this case, the gradient is set to zero.
    ...
    if(boxcox):
        ...
        n = len(beta)
        lambdas = beta[nASC+nlambda:]
        betan = beta[nASC:nASC+nlambda]
        betas = beta[0:nASC+nlambda] 
        X = np.copy(design[:,nASC:nASC+nlambda]) 
        X0 = 1*(X!=0)
        X[np.where(X==0)] = 1
        Xcox = X0*bx(X,lambdas)
        Xcox = np.concatenate((design[:,0:nASC],Xcox),axis=1)
        

        sys_utilities = Xcox.dot(betas)
   
        Xpow = np.power(X,lambdas)
        
        Xlog = np.log(X)
        X1 = np.multiply(Xlog,lambdas)
        ind0=np.where(lambdas==0)[0]
        ind1=np.where(lambdas!=0)[0]
        Xgrad=np.zeros(np.shape(X))
        Xgrad[:,ind1] = np.multiply(np.divide((Xpow[:,ind1]*(X1[:,ind1]-1)+1),lambdas[ind1]**2),betan[ind1])
        Xgrad[:,ind0]=np.multiply(0.5*np.power(np.copy(Xlog[:,ind0]),2),betan[ind0])
        
        long_probs = calc_probabilities(betas,
                                        Xcox,
                                        alt_IDs,
                                        rows_to_obs,
                                        rows_to_alts,
        ...
        dh_dc2 = transform_first_deriv_c(sys_utilities, alt_IDs,
                                        rows_to_alts, shape_params)
    
        dh_d_alpha2 = transform_deriv_alpha(sys_utilities, alt_IDs,
                                           rows_to_alts, intercept_params)
        dh_dv2 = transform_first_deriv_v(sys_utilities, alt_IDs,
                                        rows_to_alts, shape_params)

        dh_db = dh_dv.dot(Xcox) 
        dh_db2 = dh_dv.dot(Xgrad) 

   
        d_ll_dh = np.multiply(weights, choice_vector - long_probs)[np.newaxis, :]
    
        d_ll_d_beta = d_ll_dh.dot(dh_db)
        d_ll_d_lambda=d_ll_dh.dot(dh_db2)
        ...
        if shape_params is not None and intercept_params is not None:
            ...
            gradient = np.concatenate((np.asarray(d_ll_dh * hstack((dh_dc,
                                                                    dh_d_alpha),
                                                                   format='csr')),
                                       d_ll_d_beta,d_ll_d_lambda), axis=1).ravel()
            ...
            # Note that the 'np.asarray' is because dll_dh * dh_dc will be a row
            # matrix, but we want a 1D numpy array.
            gradient = np.concatenate((np.asarray(d_ll_dh * dh_dc), d_ll_d_beta,d_ll_d_lambda),
                                      axis=1).ravel()
            ...
            gradient = np.concatenate((d_ll_d_beta,d_ll_d_lambda), axis=1).ravel()
            params = beta
    
        if ((lasso is not None) and(ridge is not None)):                                               #%% ##############################
            gradient =gradient-lasso * reg.L1(params,boxcox,get_grad=True)-ridge*reg.L2(params,boxcox,get_grad=True)
        elif (lasso is not None):
              gradient =gradient-lasso * reg.L1(params,boxcox,get_grad=True)
        elif(ridge is not None):
              gradient =gradient-ridge * reg.L2(params,boxcox,get_grad=True)                                   #%% ############################### 
        #outputfile="/Users/romainpalazzo/Desktop/Semestre 7/Machine Learning/Projet_2/grad.mat"
        #sio.savemat(outputfile, mdict={'grad':gradient})
        #print(np.linalg.norm(gradient))
        gradient[nASC+nlambda+np.where(lambdas==0)[0]]=0
        #print(np.linalg.norm(gradient))
        return gradient
 ...
 ######################## END MODIFIED LINES #################################
    ...
    # alternative for that individual.
        ...
        """
 ######################## MODIFIED LINES #####################################
 Computes the gradient of the log-likelihood with regularisation. Depending on 
 if 'lasso' or 'ridge' input parameter are defined, adds a L1, L2 
 or the two gradient of regularisations.
      """
        if ((lasso is not None) and(ridge is not None)):
            gradient =gradient-lasso * reg.L1(params,boxcox,get_grad=True)-ridge*reg.L2(params,boxcox,get_grad=True)
        elif (lasso is not None):
