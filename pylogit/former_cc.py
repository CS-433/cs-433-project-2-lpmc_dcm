...
def calc_probabilities(beta,
    ...
    if(boxcox):
        ...
        n=len(beta)
        lambdas=np.copy(beta[nASC+nlambda:]) #a revoir
        betas=np.copy(beta[0:nASC+nlambda]) #a revoir
        X=np.copy(design[:,nASC:nASC+nlambda]) #a revoir
        X0=1*(X!=0)
        X[np.where(X==0)] = 10
        Xcox_tmp=X0*bx(X,lambdas)
#        Xcox_tmp=X0 * st.boxcox(X,lambdas)
#        outputfile="C:\\Users\\Eliott\\Desktop\\ML\\project_2\\X.mat"
#        sio.savemat(outputfile, mdict={'X':X})
#        outputfile2="C:\\Users\\Eliott\\Desktop\\ML\\project_2\\lmda.mat"
#        sio.savemat(outputfile2, mdict={'lmda':lambdas})
        
        Xcox=np.concatenate((design[:,0:nASC],Xcox_tmp),axis=1)
...
def calc_log_likelihood(beta,
    ...
    if (ridge is None) and (ridge2 is None):                                                #####################################
        ...
        if len(param_list) > 1:
            ...
            params = param_list[0]
            #print(ridge*reg.L1(params))
        if (ridge is not None) and ((ridge2 is not None)):                              #%%  ####################################                       
            
            return log_likelihood - ridge*reg.L1(params,boxcox)-ridge2*reg.L2(params)            
        elif(ridge is not None):
            return log_likelihood - ridge*reg.L1(params,boxcox)
        elif(ridge2 is not None):
            return log_likelihood - ridge2*reg.L2(params,boxcox)                              #%%  #####################################
...
def calc_gradient(beta,
    ...
    if(boxcox):
        ...
        nlambda = int((len(beta)-nASC)/2)
        n = len(beta)
        lambdas = np.copy(beta[nASC+nlambda:])
        betan = np.copy(beta[nASC:nASC+nlambda])
        betas = np.copy(beta[0:nASC+nlambda]) #a revoir
        X = np.copy(design[:,nASC:nASC+nlambda]) #a revoir
        X0 = 1*(X!=0)
        X[np.where(X==0)] = 1
        Xcox_tmp = X0*bx(X,lambdas)
        #Xcox = X0*st.boxcox(X,lambdas)
        Xcox = np.concatenate((design[:,0:nASC],Xcox_tmp),axis=1)
        

        sys_utilities = Xcox.dot(betas)
   
        Xpow = np.power(np.copy(X),lambdas)
        Xlog = np.log(np.copy(X))
        X1 = np.multiply(np.copy(Xlog),lambdas)
        ind0=np.where(lambdas==0)[0]
        ind1=np.where(lambdas!=0)[0]
        Xgrad=np.ones(np.shape(X))
...
#        outputfile="C:\\Users\\Eliott\\Desktop\\ML\\project_2\\Xgrad.mat"
#        sio.savemat(outputfile, mdict={'Xgrad':Xgrad})

#        X1 = np.multiply(Xlog,lambdas)
#       
#        Xgrad = np.multiply(np.divide((Xpow*(X1-1)+1),lambdas**2),betan)
#        Xgrad=np.concatenate((design[:,0:nASC],Xgrad),axis=1)
        ...
        if ((ridge is not None) and(ridge2 is not None)):                                               #%% ##############################
            gradient =gradient-ridge * reg.L1(params,boxcox,get_grad=True)-ridge2*reg.L2(params,boxcox,get_grad=True)
        elif (ridge is not None):
              gradient =gradient-ridge * reg.L1(params,boxcox,get_grad=True)
        elif(ridge2 is not None):
              gradient =gradient-ridge2 * reg.L2(params,boxcox,get_grad=True)                                   #%% ############################### 
